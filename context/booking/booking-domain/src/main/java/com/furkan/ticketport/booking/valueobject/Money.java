...
import com.furkan.ticketport.booking.exception.InvalidAmountException;
...
public final class Money {
    ...
    /** Birim fiyat × adet (ücretsiz seansta birim {@link #free} ise sonuç yine sıfırlı aynı para biriminde olur). */
    public Money multiply(int quantity) {
        if (quantity < 1) {
            throw new InvalidAmountException("Quantity must be at least 1");
        }
        BigDecimal product = amount.multiply(BigDecimal.valueOf(quantity));
        return new Money(copyNormalized(product, currency), currency);
    }
